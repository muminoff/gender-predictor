...
uzbek_alphabet = [letter for letter in uzbek_alphabet] + ['o‘', 'g‘', 'sh', 'ch']
...
class IsmcrawlerSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        boys_name_urls = [boys_name_base_url.format(letter=letter) for letter in uzbek_alphabet]
        girls_name_urls = [girls_name_base_url.format(letter=letter) for letter in uzbek_alphabet]
        all_urls = boys_name_urls + girls_name_urls
        return [scrapy.Request(url=url) for url in all_urls]

    def parse(self, response):
        sanitize = lambda items: [i.replace('\n', '').replace('\t', '').strip().split()[0].lower() for i in items]
        ismlar_raw = scrapy.selector.Selector(response).xpath('/html/body/div[2]/div[1]/div/div/ul/li/a/text()').extract()
        ismlar_raw = scrapy.selector.Selector(response).xpath('/html/body/div[2]/div[1]/div/div/ul/li/a/text()').extract()
        ismlar = sanitize(ismlar_raw)
        
        for ism in ismlar:
            gender = lambda url: 'male' if 'boy' in url else 'female'
            item = HumanName()
            item['name'] = ism
